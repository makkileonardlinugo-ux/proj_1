...
function renderProductEmail(product, content, isPlanet, imageUrl) {
  const specRows     = product.specs.map(s => `<tr><td style="padding:9px 0; border-bottom:1px solid #1e2129; font-size:0.82rem; color:#f2f4f5;"><span style="color:#c6a559; margin-right:8px;">&#10003;</span>${s}</td></tr>`).join('');
  const bestForItems = content.best_for.map(b => `<span style="display:inline-block; margin:4px; padding:5px 12px; background:#111318; border:1px solid #1e2129; border-radius:999px; font-size:0.78rem; color:#8b9197;">${b}</span>`).join('');
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px; width:100%;">
        <tr><td style="background:#0a0c10; border:1px solid #1e2129; border-radius:16px 16px 0 0; padding:32px 36px 24px;">
          ...
          ${isPlanet ? `<div style="font-size:0.78rem; color:#8b9197; margin-bottom:6px;">Price: <span style="color:#c6a559; font-weight:700;">${product.price}</span></div>` : ''}
        ...
        ${imageUrl ? `<tr><td style="border-left:1px solid #1e2129; border-right:1px solid #1e2129; padding:0; line-height:0;"><img src="${imageUrl}" alt="${product.name}" width="600" style="width:100%;height:240px;object-fit:cover;display:block;" /></td></tr>` : ''}
        <tr><td style="background:#0a0c10; border-left:1px solid #1e2129; border-right:1px solid #1e2129; padding:28px 36px;"><div style="font-size:0.95rem; color:#f2f4f5; line-height:1.8;">${content.pitch}</div></td></tr>
        <tr><td style="background:#0d0f15; border:1px solid #1e2129; border-top:none; padding:24px 36px;">
        ...
        <tr><td style="background:#080a0e; border:1px solid #1e2129; border-top:none; border-radius:0 0 16px 16px; padding:18px 36px; text-align:center;">
